 ...
 *     entry for this table. Step-up drain iterates the manager's entries -- with no entry, the
 *     drain never resolves the claim-prepared cells on the stable side, and a post-step-up reader
 *     hits WT_PREPARE_CONFLICT.
 */
static int
...
err:
    ...
    /*
     * ENOENT can happen if the layered table no longer exists in metadata (e.g. dropped between
     * the metadata scan that surfaced it and this call). That's benign for prepared discovery --
     * there is nothing on the stable side to drain either -- so swallow it.
     */
    if (ret == ENOENT)
        ret = 0;
    return (ret);
...
__prepared_discover_walk_one_tree(WT_SESSION_IMPL *session, const char *uri)
{
    ...
    if (btree->root.page != NULL) {
        if (__prepared_discover_is_follower_stable_walk(session, uri)) {
             ...
             * follower-side touchpoint was the prepared_discover walk (no application cursor on
             * the layered URI before step-up) would otherwise be missed by drain.
             */
